    ...
    public static getInstance() {
        ...
        return {
            ...
            saveAsPNG: function (canvas: CanvasImageSource, width: number, height: number) {
                return saveAsImage(canvas, width, height, 'png', `${LocalStorageData.hashID}_share_${Date.now()}`);
            },
            saveAsJPEG: function (canvas: CanvasImageSource, width: number, height: number) {
                return saveAsImage(canvas, width, height, 'jpeg', `${LocalStorageData.hashID}_share_${Date.now()}`);
            },
            saveAsGIF: function (canvas: CanvasImageSource, width: number, height: number) {
                return saveAsImage(canvas, width, height, 'gif', `${LocalStorageData.hashID}_share_${Date.now()}`);
            },
            saveAsBMP: function (canvas: CanvasImageSource, width: number, height: number) {
                return saveAsImage(canvas, width, height, 'bmp', `${LocalStorageData.hashID}_share_${Date.now()}`);
            },
