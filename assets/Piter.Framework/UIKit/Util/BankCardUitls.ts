...
export class BankCardUtils {
  ...
  public static jiamiCard(bankCardNumber: string) {
    if (bankCardNumber.length < 0) {
      return "未知";
    }
    const groups = bankCardNumber.match(/.{1,4}/g);
    let backStr = "";
  ...
  private static getCardTypeName(cardType) {
  ...
  static getBankNameByBankCode(bankcode) {
  ...
  static getBankInfoByCardNo(cardNo) {
    ...
    for (var i = 0, len = this.bankcardList.length; i < len; i++) {
      ...
      for (var j = 0, jLen = patterns.length; j < jLen; j++) {
        ...
        if ((new RegExp(pattern.reg)).test(cardNo)) {
          ...
          var info = this.extend(bankcard, pattern) as any;

          delete info.patterns;

          delete info.reg;

          info['cardTypeName'] = this.getCardTypeName(info['cardType']);
