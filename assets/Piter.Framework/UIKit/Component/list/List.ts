...
export default class List extends Component {
    ...
    @property({
        type: EventHandler,
        tooltip: DEV && '滚动开始事件监听',
    })
    public scrollBeganEvent: EventHandler = new EventHandler();
    @property({
        type: EventHandler,
        tooltip: DEV && '滚动停止事件监听',
    })
    ...
    public touchCancelEvt: EventHandler = new EventHandler();
    //选择模式
    @property({
    ...
    public displayItemNum: number; //界面上展示的item数量
    ...
    _onScrollBegan() {
        ...
        if (this.scrollBeganEvent) {
            EventHandler.emitEvents([this.scrollBeganEvent]);
        }
    }
    //滚动结束时..
    _onScrollEnded() {
    ...
    _onTouchCancelled(ev, captureListeners) {
        ...
        this._scrollItem = null;
        if (this.touchCancelEvt) {
            EventHandler.emitEvents([this.touchCancelEvt]);
        }
    }
    //当尺寸改变
    _onSizeChanged() {
    ...
    _pageAdhere() {
        ...
        if (canSkip) {
            ...
            switch (t._alignCalcType) {
                case 1://单行HORIZONTAL（LEFT_TO_RIGHT）、网格VERTICAL（LEFT_TO_RIGHT）
                case 4://单列VERTICAL（BOTTOM_TO_TOP）、网格HORIZONTAL（BOTTOM_TO_TOP）
                    let pageDvalue = Math.round(Math.abs(t._beganPos - curPos) / dis);
                    if (t._beganPos > curPos) {
                        t.prePage(timeInSecond * pageDvalue, pageDvalue);
                        // cc.log('_pageAdhere   PPPPPPPPPPPPPPP');
                    } else {
                        t.nextPage(timeInSecond * pageDvalue, pageDvalue);
    ...
    scrollTo(listId: number, timeInSecond: number = .5, offset: number = null, overStress: boolean = false) {
        ...
        if (t.pageChangeEvent) {
            EventHandler.emitEvents([t.pageChangeEvent], listId);   //页面变更
        }
    ...
    //上一页
    prePage(timeInSecond: number = .5, pageDvalue: number = 1) {
        // cc.log('👈');
        if (!this.checkInited())
            return;
        this.skipPage(this.curPageNum - pageDvalue, timeInSecond);
    }
    //下一页
    nextPage(timeInSecond: number = .5, pageDvalue: number = 1) {
        // cc.log('👉');
        if (!this.checkInited())
            return;
        this.skipPage(this.curPageNum + pageDvalue, timeInSecond);
    ...
    skipPage(pageNum: number, timeInSecond: number) {
        ...
        if (pageNum >= t._numItems) {
            pageNum = t._numItems - 1
        }
